...
def DBR_solar_tmm(d_Stack):
    ...
    num_layers = (
        len(d_Stack) + 2
    )  # need to add 1 "layer" in tmm, for include the air above and upper the thin layer stack
    num_stacks = 1
    # create m
...
instrum = ng.p.Instrumentation(
    ng.p.Array(shape=(num_layer,)).set_bounds(lower=0, upper=300)
)
optimizer = ng.optimizers.DE(
    parametrization=instrum, budget=budget
)  # We use DE optimizer, in nevergrad
...
print(
    "Value of cost function for SolPOC:", DBR_solar_SolPOC(recommendation.value[0][0])
)
print(
    "Solution (Thin layers thicknesses, in nm) with SolPOC ", recommendation.value[0][0]
)
